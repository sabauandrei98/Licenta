...
class Server:
	...
	def unity_client_handler(self, client_socket, addr):
		...
		try:
			...
			unity_token = client_socket.recv(RECV_SIZE_BYTES).rstrip('\r\n')

			print(unity_token)
			if unity_token == UNITY_TOKEN:
				client_socket.send("Ok!")
			else:
				raise Exception ("An IDE tried to connect while in lobby section !")
			...
			print("This socket was closed ! A new unity client can reconnect on this socket and token !")
